...
// auto-loads .env). DEFAULT "observe" for the Phase-0 landing (spec §7 Phase 0:
// land the liveness watchdog log-only first, flip to enforce after a clean
// observe day, §12). Set S1S2_WATCHDOG_MODE=enforce to activate WITHOUT a code
// change. VoiceModule's response-watchdog reads the same env var independently.
