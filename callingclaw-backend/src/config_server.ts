        ...
        (async () => {
          try {
            ...
            const taskPrompt = [
              ...
              `**特别重要：从 MEMORY.md 的 Lessons Learned 和历史记录中找出与本次话题相关的过去犯错、失败和教训。**`,
              `这些 learning 必须在 prep 文档中明确体现，确保同样的错误不会重复发生。`,
              ``,
              ...
              ``,
              `Markdown 自由格式，**必须包含**:`,
              `# 标题, 目标, 概要, 要点, 架构决策, 预期问题, **⚠️ 过去的教训和踩坑记录**, 历史背景, 相关文件和链接`,
              ``,
