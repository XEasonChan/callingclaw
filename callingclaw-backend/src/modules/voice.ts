...
// S1S2_WATCHDOG_MODE env var (Bun auto-loads .env), DEFAULT "enforce" — the
// branch's goal is ACTIVE watchdogs. Setting S1S2_WATCHDOG_MODE=observe reverts
// both watchdogs to log-only WITHOUT a code change (dogfood-a-day-then-flip,
// §12). realtime_client.ts reads the same env var independently.
...
export class VoiceModule {
   ...
   * private client, no phantom method (the previous call site invoked a
   * nonexistent client.queuePendingResponse() and threw a TypeError in the
   * common non-idle case, which injected a FALSE "failed" note while the real
   * answer sat unspoken in Layer 3).
   *
