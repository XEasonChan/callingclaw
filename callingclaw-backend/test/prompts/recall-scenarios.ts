/**
 * Recall Context Scenario Data — 50 realistic mid-meeting context retrieval cases.
 *
 * Data + interface ONLY, no test imports. Imported by BOTH
 * test/prompts/recall-context-scenarios.test.ts (structural validation + E2E
 * keyword eval) and test/prompts/auditor-intent-eval.test.ts (recall lane of
 * the intent eval). Keeping the data in a plain .ts module means importing it
 * from one eval does NOT re-register the other file's describe blocks
 * (importing a .test.ts file registers its tests in the importer's run).
 */
