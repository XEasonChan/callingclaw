...
- **Codex** (OpenAI): `./scripts/setup-codex.sh` locates the Codex CLI and registers the MCP server in `~/.codex/config.toml`. Adapter shells out to `codex exec "<prompt>"` (per-run model via `-m`, defaulting to `~/.codex/config.toml`). Talk to it via `codex exec "..."`.
...
- **Raven** (EverMind-AI): install Raven yourself first (`pipx install raven-agent`), then `./scripts/setup-raven.sh` locates it, injects the `mcp` client dependency into Raven's own env (Raven doesn't declare it — without it MCP silently registers *zero* tools), seeds an OpenRouter provider + model into `~/.raven/config.json`, and registers the MCP server. Talk to it via `./scripts/start-raven.sh`. Live E2E: `bun scripts/e2e-raven.ts`. Adapter shells out to `raven agent -m "<prompt>"`; model/provider are config-file based (`~/.raven/config.json`, NOT CLI flags). Design: `docs/raven-adapter-design.md`.
