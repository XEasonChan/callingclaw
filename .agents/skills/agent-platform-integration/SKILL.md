...
- [ ] **(c) Adapter** — create `src/adapters/<name>-adapter.ts` implementing **every** `AgentAdapter` member. Copy the closest reference adapter and swap the runner. Full member list (all required except `onActivity`):
